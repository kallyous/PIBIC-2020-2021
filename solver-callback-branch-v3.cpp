...
ILOBRANCHCALLBACK3(
    ...
    // Prepara
    IloNumArray x_val = IloNumArray(getEnv());
    IloNumArray y_val = IloNumArray(getEnv());
    IntegerFeasibilityArray x_feas = IntegerFeasibilityArray(getEnv());
    IntegerFeasibilityArray y_feas = IntegerFeasibilityArray(getEnv());
    ...
    for (IloInt i = 0; i < cols; i++) {
        ...
        if (x_feas[i] == Infeasible)
        {
            ...
            xi_rel = weights[i] / abs(0.5 - x_val[i]);
        ...
        if (y_feas[i] == Infeasible)
        {
            ...
            yi_rel = weights[i] / abs(0.5 - y_val[i]);
