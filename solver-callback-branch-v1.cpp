...
ILOBRANCHCALLBACK3(
    ...
    IloInt x_max_inf = 0.0;
    IloInt y_max_inf = 0.0;
    IloInt x_max_weight = 0.0;
    IloInt y_max_weight = 0.0;
    ...
    for (IloInt i = 0; i < cols; i++) {
        ...
        {
            // Pega e trata o valor de x para o algoritmo.
            IloNum xi_inf = x_val[i];
            if (xi_inf > 0.5) xi_inf = 1.0 - xi_inf;

            // Aplica critério de escolha e atualiza os índices.
            if ( xi_inf >= x_max_inf
              && (xi_inf > x_max_inf || weights[i] >= x_max_weight) )
            {
                best_i = i;
                x_max_inf = xi_inf;
                x_max_weight = weights[i];
            }
        }
        ...
        {
            // Pega e trata o valor de y para o algoritmo.
            IloNum yi_inf = y_val[i];
            if (yi_inf > 0.5) yi_inf = 1.0 - yi_inf;

            // Aplica critério de escolha e atualização dos índices.
            if ( yi_inf >= y_max_inf
              && (yi_inf > y_max_inf || weights[i] >= y_max_weight) )
            {
                best_j = i;
                y_max_inf = yi_inf;
                y_max_weight = weights[i];
            }
        }
    }
